...
export const CertificateDemo: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-stone-100 py-12 md:py-20 print:p-0 print:bg-white">
      {/* Elementos da Interface - Escondidos na impressão */}
      <div className="max-w-[1400px] mx-auto px-4 print:hidden no-print">
        <div className="text-center mb-12">
          ...
          <p className="mt-4 text-stone-500 font-medium tracking-wide">合格おめでとうございます。内容を確認して、印刷または保存してください。</p>
        </div>
        ...
        <div className="grid lg:grid-cols-12 gap-12 items-start">
          ...
          {/* Área do Certificado */}
          <div className="lg:col-span-8 order-1 lg:order-2 certificate-printable-container">
             <div className="sticky top-32">
                <div className="mb-4 flex items-center justify-between px-4 print:hidden">
                  <span className="text-xs font-bold text-stone-400 tracking-widest uppercase">Live Preview (A4 Aspect)</span>
                  <div className="text-[10px] text-green-600 font-bold bg-green-50 px-2 py-1 rounded border border-green-100">Ready to Print</div>
                </div>

                {/* Canvas Principal do Certificado */}
                <div 
                ...
                >
                  ...
                  <div className="absolute inset-0 p-[2.5%] pointer-events-none">
                     <div className={`w-full h-full border-[8px] border-double ${selectedCourseId === 'regular' ? 'border-orange-100' : selectedCourseId === 'senior' ? 'border-teal-50' : 'border-indigo-50'} rounded-sm`}></div>
                     <div className={`absolute inset-[3.5%] border-[1px] ${selectedCourseId === 'regular' ? 'border-orange-200' : selectedCourseId === 'senior' ? 'border-teal-100' : 'border-indigo-100'} opacity-30`}></div>
                  </div>

                  {/* Cantos Decorativos */}
                  <div className={`absolute top-[4.5%] left-[4.5%] w-8 h-8 border-t-2 border-l-2 ${selectedCourseId === 'regular' ? 'border-orange-300' : 'border-stone-200'}`}></div>
                  <div className={`absolute top-[4.5%] right-[4.5%] w-8 h-8 border-t-2 border-r-2 ${selectedCourseId === 'regular' ? 'border-orange-300' : 'border-stone-200'}`}></div>
                  <div className={`absolute bottom-[4.5%] left-[4.5%] w-8 h-8 border-b-2 border-l-2 ${selectedCourseId === 'regular' ? 'border-orange-300' : 'border-stone-200'}`}></div>
                  <div className={`absolute bottom-[4.5%] right-[4.5%] w-8 h-8 border-b-2 border-r-2 ${selectedCourseId === 'regular' ? 'border-orange-300' : 'border-stone-200'}`}></div>

                  {/* Conteúdo Interno - Reduzi os elementos e aumentei o espaçamento inferior para evitar cortes */}
                  <div className="absolute inset-0 flex flex-col items-center justify-between py-[5%] px-[12%] text-center">
                    ...
                    {/* Título Principal */}
                    <div className="w-full">
                       <h2 className="text-2xl md:text-4xl font-bold text-stone-800 font-serif-jp tracking-[0.2em] mb-2">認定証</h2>
                       <div className={`w-20 h-0.5 ${selectedCourse.color} mx-auto rounded-full`}></div>
                       <div className="mt-2 space-y-0.5">
                          <div className="text-[9px] md:text-xs font-bold text-stone-500 font-serif-jp">{selectedCourse.level}</div>
                          <div className={`text-xs md:text-lg font-bold ${selectedCourse.accent} font-serif-jp tracking-wide uppercase`}>
                    ...
                    <div className="w-full my-1">
                       <div className="text-stone-400 text-[7px] md:text-[10px] font-bold mb-1">認定者</div>
                       <div className="relative inline-block px-10 pb-0.5">
                          <div className="text-xl md:text-3xl font-bold text-stone-900 font-serif-jp">
                             {userName || '　　　　'} <span className="text-sm md:text-lg ml-3 text-stone-500 font-normal">殿</span>
                          </div>
                    ...
                    <div className="w-full">
                       <p className="text-[8px] md:text-[11px] lg:text-sm text-stone-600 font-serif-jp leading-relaxed md:leading-loose max-w-[90%] mx-auto">
                    ...
                    {/* Rodapé e Selo - Ajustado para segurança extra no inferior */}
                    <div className="w-full flex justify-between items-end pt-3 border-t border-stone-100">
                       <div className="text-left space-y-2">
                          <div>
                             <div className="text-[6px] md:text-[7px] font-bold text-stone-400">発行日</div>
                             <div className="text-[9px] md:text-xs font-bold text-stone-800 font-serif-jp">2026年3月15日</div>
                          </div>
                          <div>
                             <div className="text-[6px] md:text-[7px] font-bold text-stone-400">登録番号</div>
                       ...
                       <div className="flex items-center gap-4 md:gap-6">
                          <div className="text-right">
                             <div className="text-[6px] md:text-[7px] font-bold text-stone-400 mb-0.5">一般社団法人 日本ダイエットアカデミー協会</div>
                          ...
                          <div className={`relative w-12 h-12 md:w-16 md:h-16 rounded-full border-2 border-double flex items-center justify-center bg-white ${
                            ...
                            'border-indigo-500/40 text-indigo-600'
                             ...
                             <AwardIcon size={24} className="md:w-8 md:h-8 opacity-90" strokeWidth={1} />
                             <div className="absolute inset-0 flex items-center justify-center text-[4px] font-bold opacity-10 uppercase pointer-events-none tracking-tighter">
                                JDAA Seal
                             </div>
                          </div>
                       </div>
                  ...
                  {isIssuing && (
                    <div className="absolute inset-0 bg-white/95 backdrop-blur-sm flex flex-col items-center justify-center animate-in fade-in duration-300 z-50">
                      ...
                      <div className="text-lg font-bold text-stone-800 font-serif-jp">情報を更新中...</div>
                    </div>
                  )}
                </div>
      ...
      <style>{`
        ...
        @media print {
          /* 1. Reset Total para evitar páginas em branco */
          html, body {
            margin: 0 !important;
            padding: 0 !important;
          ...
          /* 2. Ocultar TUDO exceto o certificado e seus pais diretos */
          header, footer, aside, .no-print, .controls-panel, .scroll-to-top-button {
            display: none !important;
            height: 0 !important;
            overflow: hidden !important;
          }

          /* 3. Garantir que o certificado ocupe a página inteira */
          ...
          /* 4. Forçar o canvas a preencher o espaço exato */
          #certificate-content {
          ...
          /* 5. Garantir que os elementos pais não escondam o conteúdo */
          #root, #root > div, main {
            display: block !important;
